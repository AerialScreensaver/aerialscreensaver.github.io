...
What's new :
- Fix playback not being smooth, particularly with 30 fps videos. 
- Fix playlist changes not working in the extension
- Fix changing from a large playlist to a small one not refreshing the playlist correctly 
- Add loop-1 video playback, if you want to loop on one video without changing your playlist
- Add the ability to delete videos from the cache, you can right click videos to do so. 
- Add the ability to manually pick the workaround to use for overlap bug debugging (in Settings > Advanced). C may be the best in early testing.
- Various fixes
