...
What's new :
- Bring audio playback back for local videos. It's disabled by default, you will need to enable it in Advanced. It will only work when playing at 1x speed (so screensaver, and wallpaper if at 1x)
- Add audio playback volume control 
- Possible fix for crash when churning sessions for some reason
- Better debugging for potential churnings 
- Keep up to 2 extra logs just in case instead of wiping them
- Better export log diagnostics, will also try to grab crash logs if they exist
